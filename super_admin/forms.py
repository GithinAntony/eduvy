...
class SignInForm(forms.Form):
    ...
    def clean_email(self):
        ...
        in use.
        """
        existing = superAdmin.objects.filter(email=self.cleaned_data['email'])
        if existing.exists():
...
class AddCollegeForm(forms.Form):
    ...
    website = forms.CharField(max_length=200, widget=forms.TextInput(
        attrs={'autocomplete': 'off', 'class': 'form-control', 'placeholder': 'Website'}))
    phone = forms.CharField(max_length=200, widget=forms.TextInput(
        attrs={'autocomplete': 'off', 'class': 'form-control', 'placeholder': 'Phonenumber'}))
    email = forms.EmailField(max_length=254, widget=forms.TextInput(
        attrs={'autocomplete': 'off', 'class': 'form-control', 'placeholder': 'Email'}))
    address = forms.CharField(max_length=600, widget=forms.Textarea(
        attrs={'autocomplete': 'off', 'class': 'form-control', 'placeholder': 'Address', 'rows': '3'}))
...
class AddStateForm(forms.Form):
    state_name = forms.CharField(max_length=200, widget=forms.TextInput(
        attrs={'autocomplete': 'off', 'class': 'form-control', 'placeholder': 'State'}))
    state_code = forms.CharField(max_length=200, widget=forms.TextInput(
        attrs={'autocomplete': 'off', 'class': 'form-control', 'placeholder': 'Code'}))

class AddCityForm(forms.Form):
    city_state = forms.ModelChoiceField(queryset=State.objects.all(), to_field_name='unique_id',
                                            empty_label="Select State",
                                            widget=forms.Select(attrs={'class': 'form-control'}))
    city_name = forms.CharField(max_length=200, widget=forms.TextInput(
        attrs={'autocomplete': 'off', 'class': 'form-control', 'placeholder': 'City'}))
...
class CourseStreamForm(forms.Form):
    name = forms.CharField(max_length=100, widget=forms.TextInput(
        attrs={'autocomplete': 'off', 'class': 'form-control', 'placeholder': 'Course Stream'}))
    description = forms.CharField(max_length=500, widget=forms.Textarea(
        attrs={'autocomplete': 'off', 'class': 'form-control', 'placeholder': 'Description', 'rows': '3'}))
