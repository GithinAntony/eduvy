...
class SignInForm(forms.Form):
    ...
    def clean_email(self):
        ...
        in use.
        """
        existing = collegeOwner.objects.filter(email=self.cleaned_data['email'])
        if existing.exists():
...
class EditProfile(forms.Form):
    ...
    address = forms.CharField(max_length=600, widget=forms.Textarea(
        attrs={'autocomplete': 'off', 'class': 'form-control', 'placeholder': 'Address', 'rows': '3'}))
    profile_photo = forms.FileField(validators=[FileExtensionValidator(['jpg', 'png', 'jpeg'])])
    ...
    def clean_email(self):
        ...
        in use.
        """
        existing = collegeOwner.objects.filter(email=self.cleaned_data['email']).exclude(unique_id=self._user_id)
        if existing.exists():
            raise forms.ValidationError(("An user with that email already exists."))
        else:
            return self.cleaned_data['email']
...
class AddCollegeForm(forms.Form):
    ...
    address = forms.CharField(max_length=600, widget=forms.Textarea(
        attrs={'autocomplete': 'off', 'class': 'form-control', 'placeholder': 'Address', 'rows': '3'}))
    select_state = forms.ModelChoiceField(queryset=State.objects.all(), to_field_name='unique_id',
                                            empty_label="Select state",
                                            widget=forms.Select(attrs={'class': 'form-control'})
                                            )
    select_city = forms.ModelChoiceField(queryset=City.objects.all(), to_field_name='unique_id',
