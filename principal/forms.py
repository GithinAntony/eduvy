...
class SignInForm(forms.Form):
    ...
    select_college = forms.ModelChoiceField(queryset=colleges.objects.all(), to_field_name='unique_id', empty_label="Select College", widget=forms.Select(attrs={'class':'form-control'})
)
    password = forms.CharField(max_length=100, widget=forms.PasswordInput(
        attrs={'autocomplete': 'off', 'class': 'form-control', 'placeholder': 'Password'}))
    ...
    def clean_email(self):
        ...
        in use.
        """
        existing = principal.objects.filter(email=self.cleaned_data['email'])
        if existing.exists():
